...
def get_badge_color(estado_actualizacion):
    ...
    elif estado_actualizacion == 'DESACTUALIZADO': return '#e67e22' # Naranja/rojo suave
    elif estado_actualizacion == 'SIN_FECHA_REPORTE': return '#95a5a6' # Gris
    elif estado_actualizacion == 'SIN_AVANCE_FISICO': return '#bdc3c7' # Gris claro
    return '#bdc3c7'

def main():
    print("Cargando datos V6...")
    ...
    FUERA_EPS = TOTAL_PUNO_PROY - DENTRO_EPS
    ...
    folium.GeoJson(
        ...
        tooltip=folium.GeoJsonTooltip(
            ...
            aliases=['Distrito:', 'Provincia:', 'UBIGEO:', 'Proyectos:', 'Monto Total:', '% Proy Puno:'],
            localize=True
        )
    ).add_to(m)
    ...
    eps_colors = {
        'EMSAPUNO S.A.': '#8e44ad',
        'EPS SEDAJULIACA S.A.': '#e74c3c',
        'EPS NOR PUNO S.A.': '#d35400',
        'EMAPA-YUNGUYO S.R.L.TDA': '#16a085',
        'EPS AGUAS DEL ALTIPLANO S.R.L.': '#f39c12'
    }
    ...
    <div style="position: fixed; bottom: 190px; left: 50px; width: 280px; 
         ...
         <h4 style="margin-top:0; font-size:14px; text-align:center; color: #2c3e50;">Agua y Saneamiento (Puno)</h4>
         <hr style="margin:5px 0;">
         <b>Total de proyectos:</b> {TOTAL_PUNO_PROY}<br>
         <b>Inversión total referencial:</b> {format_money(TOTAL_PUNO_MONTO)}<br>
         <b>Presupuesto ejecutado acumulado:</b> {format_money(TOTAL_PUNO_EJECUTADO)}
    </div>
    '''
